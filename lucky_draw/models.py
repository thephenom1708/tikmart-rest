...
class LuckyDraw(models.Model):
    ...
    orders_from_date = models.DateField()
...
def post_save_lucky_draw_receiver(sender, instance, *args, **kwargs):
    print(instance.orders_from_date - date.today() - timedelta(days=1))
    for user in UserModel.objects.filter(active=True):
        user_carts_sum = user.carts.filter(checkout=True, updated__gte=instance.orders_from_date).aggregate(Sum('subtotal'))['subtotal__sum'] or 0
        if user_carts_sum and user_carts_sum >= instance.order_amount_limit:
